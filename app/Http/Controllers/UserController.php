  ...
  public function show(Request $request, User $user)
    {
            ...
            if($user->is_active)
        {
            ...
            //   old quote count
                //  $QuoteCount = $user->leads()->where('quote', '1')->whereMonth('quotedate', Carbon::now()->month)->count();
                // $quotCount =  LeadDiscussion::where('created_by' , $user->id)->count();
                 $userlead = $user->leads;
                  $id =  $userlead->sortby('id')->pluck('id');
                //   $QuoteCount  = Proposal::where('lead_id' , $id)->whereMonth('created_at', Carbon::now()->month)->count();
               
            $created_at=Lead::where('created_by', $user->id)->get('created_at');
            $month = $request->month;
            $time=strtotime($month);
            $montht=date("m",$time);
            $year=date("Y",$time);
            // $mon =$user->leads()->whereMonth('quotedate', $montht)->whereYear('quotedate', $year)->where('quote', '1')->count();
            //  $mon = Proposal::where('lead_id' , $id)->whereMonth('created_at', $montht)->whereYear('created_at', $year)->count();
            // $lastmon =$user->leads()->whereMonth(
            //     'quotedate', '=', Carbon::now()->subMonth()->month
            // )->where('quote', '1')->count();
            //   $lastmon =Proposal::where('lead_id' , $id)->whereMonth(
           ...
           $results = $getlead->sortby('whoishe')->pluck('whoishe')->unique();
            $name =  $userlead->sortby('name')->pluck('name')->unique();
              $id =  $user->leads()->get()->pluck('id')->toArray();
            $quote = Proposal::whereIn('lead_id', $id)->count();
        
            return view('user.show', compact('user', 'calenderTasks','users','leadCount','name','results', 'quote'));
