...
class JournalEntryController extends Controller
{
    ...
    public function balanceindex()
    {
        if(\Auth::user()->can('manage journal entry'))
        {
            $journalEntries = BalanceEntry::where('created_by', '=', \Auth::user()->creatorId())->get();

            return view('balancentry.index', compact('journalEntries'));
        }
        else
        {
            return redirect()->back()->with('error', __('Permission denied.'));
    ...
    public function balancecreate()
    {
        if(\Auth::user()->can('create journal entry'))
        {
            ...
            return view('balancentry.create', compact('accounts', 'journalId'));
        }
        else
        {
            return response()->json(['error' => __('Permission denied.')], 401);
    ...
    public function balancestore(Request $request)
    {
        ...
        if(\Auth::user()->can('create invoice'))
        {
            ...
            for($i = 0; $i < count($accounts); $i++)
            {
                ...
                $journalItem              = new BalanceItem();
                $journalItem->balance     = $journal->id;
                $journalItem->account     = $accounts[$i]['account'];
                $journalItem->totalbalance     = $accounts[$i]['amount'];
                $journalItem->discription = $accounts[$i]['discription'];
                $journalItem->revenue       = isset($accounts[$i]['debit']) ? $accounts[$i]['debit'] : 0;
                $journalItem->expense      = isset($accounts[$i]['credit']) ? $accounts[$i]['credit'] : 0;
              
                $journalItem->save();
            }

            return redirect()->route('balance-entry.index')->with('success', __('Balance entry successfully created.'));
        }
        else
        {
            return redirect()->back()->with('error', __('Permission denied.'));
    ...
    {
        $journalEntry = BalanceEntry::find($id);
        if(\Auth::user()->can('show journal entry'))
        {
            if($journalEntry->created_by == \Auth::user()->creatorId())
            {
                ...
                return view('balancentry.view', compact('journalEntry', 'accounts', 'settings'));
            }
            else
            {
                return redirect()->back()->with('error', __('Permission denied.'));
    ...
    public function ledgerSummary(Request $request)
    {
        if(\Auth::user()->can('ledger report'))
        {
            ...
            $journalItems = BalanceItem::select('balance_entries.id', 'balance_entries.date as transaction_date', 'balance_items.*')->leftjoin('balance_entries', 'balance_entries.id', 'balance_items.balance')->where('balance_entries.created_by', '=', \Auth::user()->creatorId())->where('account', !empty($account) ? $account->id : 0);
            ...
            $filter['expense']         = $credit;
            $filter['revenue']          = $debit;
            $filter['startDateRange'] = $start;
            $filter['endDateRange']   = $end;


            return view('balancentry.summary', compact('filter', 'journalItems', 'account', 'accounts'));
        }
        else
        {
            return redirect()->back()->with('error', __('Permission Denied.'));
