...
class ProposalController extends Controller
{
    ...
    public function create($customerId)
    {
     ...
     public function create2($customerId)
    {
        if(\Auth::user()->can('create proposal'))
        {
            $customFields    = CustomField::where('created_by', '=', \Auth::user()->creatorId())->where('module', '=', 'proposal')->get();
            $proposal_number = \Auth::user()->proposalNumberFormat($this->proposalNumber());
                       $customers       = Customer::where('lead_id', $customerId)->where('created_by', \Auth::user()->creatorId())->first();
            ...
            return view('proposal.create2', compact('customers', 'proposal_number', 'product_services', 'category', 'customFields', 'customerId'));
        }
        else
        {
            return response()->json(['error' => __('Permission denied.')], 401);
    ...
    public function store(Request $request)
    {
        ...
        if(\Auth::user()->can('create proposal'))
        {
            ...
            // if($row->stage_id == 1 && $row->whoishe == "owner"){
            $row->quote = '1';
            $row->quotedate = $ldate;
            $row->update();
