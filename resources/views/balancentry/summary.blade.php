    ...
    <div class="row">
        <div class="col-sm-12">
            <div class="mt-2 " id="multiCollapseExample1">
                <div class="card">
                    <div class="card-body">
                        {{ Form::open(array('route' => array('balnce.ledger'),'method' => 'GET','id'=>'report_ledger')) }}
        ...
        <div class="row mt-2">
            <div class="col">
                ...
                <div class="card p-4 mb-4">
                    ...
                    <h7 class="text-sm mb-0">{{__('Balance Summary')}}</h7>
                </div>
            </div>
            ...
            <div class="row mt-2">
                ...
                <div class="col">
                    <div class="card p-4 mb-4">
                        ...
                        <h7 class="text-sm mb-0">{{\Auth::user()->priceFormat($filter['revenue'])}}</h7>
                    </div>
                </div>
                <div class="col">
                    <div class="card p-4 mb-4">
                        <h6 class="mb-0">{{__('Total Expense')}} :</h6>
                        <h7 class="text-sm mb-0">{{\Auth::user()->priceFormat($filter['expense'])}}</h7>
                    </div>
                </div>
                        ...
                        <div class="table-responsive">
                            <table class="table">
                                ...
                                @foreach($journalItems as  $item)
                                    <tr>
                                        <td class="Id">
                                            <a href="{{ route('balance-entry.show',$item->balance) }}" class="btn btn-outline-primary">{{ AUth::user()->journalNumberFormat($item->journal_id) }}</a>
                                        </td>
                                        ...
                                        <td>{{\Auth::user()->priceFormat($item->revenue)}}</td>
                                        <td>{{\Auth::user()->priceFormat($item->expense)}}</td>
                                        <td>
                                            @if($item->revenue>0)
                                                @php $debit+=$item->revenue @endphp
                                            @else
                                                @php $credit+=$item->expense @endphp
                                            @endif

                                            @php $balance= $item->totalbalance @endphp
                                            @if($balance>0)
